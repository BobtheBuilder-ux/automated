...
async def start_hourly_auto_apply(request: Request):
    ...
    try:
        ...
        max_applications_per_hour = data.get("max_applications_per_hour", 3)

        if not all([user_name, user_email, job_title]):
            return {"success": False, "error": "Missing required fields: user_name, user_email, job_title"}

        result = await auto_applicator.start_hourly_auto_apply(
            user_name=user_name,
            user_email=user_email,
            job_title=job_title,
            location=location,
...
@router.post("/stop-hourly-auto-apply")
async def stop_hourly_auto_apply(request: Request):
    """Stop hourly auto-application for a user/job title"""
    try:
        data = await request.json()
        user_email = data.get("user_email", "").strip()
        job_title = data.get("job_title")  # Optional - if None, stops all for user

        if not user_email:
            return {"success": False, "error": "user_email is required"}

        result = await auto_applicator.stop_hourly_auto_apply(
            user_email=user_email,
            job_title=job_title
        )
...
@router.get("/hourly-auto-apply-status/{user_email}")
async def get_hourly_auto_apply_status(user_email: str):
    """Get active hourly auto-apply schedules for a user"""
    try:
        result = await auto_applicator.get_active_schedules(user_email=user_email)
        return result

    except Exception as e:
        return {"success": False, "error": str(e)}

@router.get("/all-hourly-schedules")
async def get_all_hourly_schedules():
    """Get all active hourly auto-apply schedules (admin endpoint)"""
    try:
        result = await auto_applicator.get_active_schedules()
        return result
